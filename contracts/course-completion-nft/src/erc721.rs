...
impl Into<Vec<u8>> for Erc721Error {
    fn into(self) -> Vec<u8> {
        let msg = match self {
            Erc721Error::InvalidTokenId(_) => "InvalidTokenId",
            Erc721Error::NotOwner(_) => "NotOwner",
            Erc721Error::NotApproved(_) => "NotApproved",
            Erc721Error::TransferToZero(_) => "TransferToZero",
            Erc721Error::ReceiverRefused(_) => "ReceiverRefused",
        };
        msg.as_bytes().to_vec()
    }
}
...
impl<T: Erc721Params> Erc721<T> {
    ...
    fn require_authorized_to_spend(
        ...
        if from != owner {
            return Err("NotOwner".as_bytes().to_vec());
        }
        ...
        Err("NotApproved".as_bytes().to_vec())
    }
    ...
    fn call_receiver<S: TopLevelStorage>(
        ...
        if to.has_code() {
            ...
            let received = receiver
                ...
                .map_err(|_e| "ReceiverRefused".as_bytes().to_vec())?
                .0;

            if u32::from_be_bytes(received) != ERC721_TOKEN_RECEIVER_ID {
                return Err("ReceiverRefused".as_bytes().to_vec());
            }
        }
        Ok(())
    }
    ...
    pub fn burn(&mut self, from: Address, token_id: U256) -> Result<(), Vec<u8>> {
...
impl<T: Erc721Params> Erc721<T> {
    ...
    pub fn owner_of(&self, token_id: U256) -> Result<Address, Vec<u8>> {
        ...
        if owner.is_zero() {
            return Err("InvalidTokenId".as_bytes().to_vec());
        }
        Ok(owner)
    }
    ...
    pub fn safe_transfer_from_with_data<S: TopLevelStorage + BorrowMut<Self>>(
        ...
        if to.is_zero() {
            return Err("TransferToZero".as_bytes().to_vec());
        }
        storage
            .borrow_mut()
    ...
    pub fn approve(&mut self, approved: Address, token_id: U256) -> Result<(), Vec<u8>> {
        ...
        if msg::sender() != owner && !self.operator_approvals.getter(owner).get(msg::sender()) {
            return Err("NotApproved".as_bytes().to_vec());
        }
    ...
    pub fn supports_interface(interface: FixedBytes<4>) -> bool {
